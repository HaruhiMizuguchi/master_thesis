...
@register_model('ResNet50') # register_modelデコレータでResNet50を登録
class ResNet50(TaskNet): # ResNet50クラスを定義
    num_channels = 3
    name = 'ResNet50'

    def setup_net(self):
        model = models.resnet50(pretrained=True) # pretrained ResNet50モデルを読み込み
        model.fc = nn.Identity() # 最終層をIdentityに置き換え
        self.conv_params = model
        self.fc_params = nn.Identity()
        
        self.classifier = nn.Linear(2048, self.num_cls) # ResNet50の出力に合わせてLinear層の入力次元を2048に変更
        init.xavier_normal_(self.classifier.weight)
        self.classifier.bias.data.zero_()
