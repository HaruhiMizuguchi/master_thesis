...
class SelfFTSolver(BaseSolver):
	...
	def solve_common_amp(self, epoch, writer, round_iter, uns_conf_loader, gmm1_loader, iter_num, scaler, loss, sampling, gmm1_train=True, conf_mask=False, conf_mask_thres=0.95):  
		"""
		...
		for batch_idx in range(iter_num):
			...
			with autocast():
				...
				if loss == "all":
					total_loss = src_weight* xeloss_src + cc_weight * uns_conf_self_loss + sup_tgt_weight * xeloss_tgt + uc_weight * loss_ent
					#total_loss = src_weight* xeloss_src  + sup_tgt_weight * xeloss_tgt + uc_weight * loss_ent
				elif sampling == "wo_cc_final_all_loss_distance_and_ui_weighted_0.5":
					total_loss = src_weight* xeloss_src + sup_tgt_weight * xeloss_tgt + uc_weight * loss_ent
				elif sampling == "wo_uc_final_all_loss_distance_and_ui_weighted_0.5":
					total_loss = src_weight* xeloss_src + cc_weight * uns_conf_self_loss + sup_tgt_weight * xeloss_tgt
				else:		
					total_loss = sup_tgt_weight * xeloss_tgt
